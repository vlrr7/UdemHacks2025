...
class User(Base):
    ...
    password = Column(String)  # Pour production, utilisez un hash de mot de passe
...
# -----------------------------
def main():
    st.title("Application de Suivi de Santé et d'Activités")
    menu = ["Accueil", "Connexion", "Inscription", "Collecte des Données", "Analyse", "Social", "Gemini Predictions", "Paramètres"]
    choice = st.sidebar.selectbox("Menu", menu)

    # ----- Page d'accueil -----
    if choice == "Accueil":
        st.header("Bienvenue sur l'application de suivi de santé")
        ...
        if 'user_id' not in st.session_state:
            ...
            if st.button("Suivre"):
                ...
                if user_to_follow:
                    ...
                    exists = session.query(Follow).filter_by(follower_id=user_id, followed_id=user_to_follow.id).first()
                    if not exists:
            ...
            if followed_ids:
                ...
                for entry in entries:
                    ...
                    st.write(f"**{user.username}** a enregistré des données le {entry.date.date()}")
                    st.write(f"Pompes : {entry.pushups}, Repas : {entry.meals_count}, Eau : {entry.water_intake} L, Sommeil : {entry.sleep_hours} h")
                    st.markdown("---")
            else:
